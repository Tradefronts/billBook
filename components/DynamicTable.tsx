...
export default function DynamicTable({ title, favourite, filters, totalLabel, totalValue, columns, rows }) {
  ...
  return (
    <div
      key={i}
      className="flex items-center justify-between border rounded-xs border-gray-300 px-3 py-2  bg-white"
      style={{ width: f.width }}
    >
      ...
      <div className="border rounded-md bg-white mb-0">
        {totalLabel&&<div className="px-4 py-3 font-medium">
          {totalLabel}: <span className="font-semibold">₹ {totalValue}</span>
        </div>}
      </div>

      <div className="border border-t-0 rounded overflow-hidden">
        <table className="w-full text-xs text-gray-700">
          <thead className="bg-gray-50 text-gray-600 border-b">
            <tr>
              {columns.map((col, index) => (
                <th key={index} className="text-left px-4 py-3 text-xs font-semibold">
                  {col}
                </th>
              ))}
            </tr>
